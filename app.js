...
document.getElementById('btn-add-species').addEventListener('click', () => {
    document.getElementById('species-modal-title').innerText = 'Nova Espécie'; document.getElementById('edit-species-id').value = ''; document.getElementById('sp-nome').value = ''; document.getElementById('sp-dias').value = ''; document.getElementById('sp-temp').value = ''; document.getElementById('sp-humid').value = ''; openModal('modal-species');
});
...
// ==========================================
// 8. DASHBOARD E ANIMAÇÃO DE TEMPERATURA
// ==========================================
...
function updateDashboard() {
    ...
    if (!ativo) {
        ...
        // Para a animação se não houver lote
        if (animacaoAtiva) { clearInterval(animacaoAtiva); animacaoAtiva = false; }
        return;
    }
    ...
    const inicioDate = new Date(partes[0], partes[1] - 1, partes[2], 12, 0, 0);
    const hoje = new Date(); hoje.setFullYear(2025); hoje.setHours(12, 0, 0, 0);
    ...
    if (!animacaoAtiva && sp) {
        ...
        animacaoAtiva = setInterval(() => {
            tempoAnimacao += 0.2; // Velocidade da oscilação
            
            // Matemática de onda para temperatura (Sobe e desce cerca de 0.8 graus)
            const ondaTemp = Math.sin(tempoAnimacao) * 0.8; 
            const tempAtual = (sp.temp + ondaTemp).toFixed(1);
            
            // Matemática de onda para umidade (Oscila cerca de 5% pra cima e pra baixo)
            ...
            // Muda a cor do texto se esquentar muito ou esfriar muito
            const elTemp = document.getElementById('dash-temp');
            const diffTemp = tempAtual - sp.temp;
            if (diffTemp > 0.5) elTemp.style.color = '#e74c3c'; // Ficou quente (Vermelho)
            else if (diffTemp < -0.5) elTemp.style.color = '#3498db'; // Ficou frio (Azul)
            else elTemp.style.color = '#f0c040'; // Normal (Dourado)
...
function renderCalendar() {
    ...
    const ev = [
        { t: 'Início da Incubação', dias: 0, cor: 'var(--accent)' },
        { t: 'Ovoscopia (Dia 7)', dias: 7, cor: '#3498db' },
        { t: 'Ovoscopia (Dia 14)', dias: 14, cor: '#3498db' },
        { t: 'Parar Viragens (3 dias antes)', dias: sp.dias - 3, cor: '#e67e22' },
        { t: 'Previsão de Eclosão', dias: sp.dias, cor: 'var(--success)' }
    ];
    c.innerHTML = ev.map(e => {
        const dt = new Date(ini.getTime() + (e.dias * d)), df = dt.toLocaleDateString('pt-BR', { weekday: 'long', day: '2-digit', month: 'long', year: 'numeric' }), past = dt.getTime() < new Date().setHours(0,0,0,0);
        return `<div class="card-item" style="border-left: 4px solid ${e.cor}; opacity: ${past ? 0.5 : 1};"><div><h4 style="color: ${e.cor}; margin-bottom: 5px;">${e.t}</h4><p style="color: var(--text-dim); text-transform: capitalize;">${df} ${past ? '(Passado)' : ''}</p></div></div>`;
    }).join('');
}
...
function getAppContext() { const a = lotes.find(l => l.ativo); if (!a) return "AVISO: O usuário NÃO possui nenhum lote ativo."; const sp = species.find(s => s.id === a.especieId), p = a.dataInicio.split('-'), ini = new Date(p[0], p[1] - 1, p[2], 12, 0, 0), ho = new Date(); ho.setFullYear(2025); ho.setHours(12,0,0,0); const da = Math.floor((ho.getTime() - ini.getTime()) / (1000*60*60*24)), df = Math.max(0, (sp ? sp.dias : 21) - da); let c = 'DADOS_DO_APP:\nLote: '+a.nome+'\nEspecie: '+(sp?sp.nome:'?')+'\nOvos: '+a.qtd+'\nTemperatura Alvo: '+(sp?sp.temp+'°C':'?')+'\nUmidade Alvo: '+(sp?sp.umid+'%':'?')+'\nDia Atual: '+da+'\nDias Restantes: '+df+'\nFIM_DADOS.'; return c; }
