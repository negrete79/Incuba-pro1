...
function renderLotes() {
    ...
    list.innerHTML = lotes.map(l => {
        ...
        const partes = l.dataInicio.split('-');
        const dataFormatada = new Date(partes[0], partes[1] - 1, partes[2], 12, 0, 0).toLocaleDateString('pt-BR');
...
function updateDashboard() {
    ...
    if (!animacaoAtiva && sp) {
        ...
        animacaoAtiva = setInterval(() => {
            ...
            if (diffTemp > 0.5) {
                elTemp.style.color = '#e74c3c'; // Ficou quente (Vermelho)
            } else if (diffTemp < -0.5) {
                elTemp.style.color = '#3498db'; // Ficou frio (Azul)
            } else {
                elTemp.style.color = '#f0c040'; // Normal (Dourado)
            }
...
function renderCalendar() {
    ...
    const partes = ativo.dataInicio.split('-');
    const inicio = new Date(partes[0], partes[1] - 1, partes[2], 12, 0, 0);
